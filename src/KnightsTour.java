...
public class KnightsTour {
    ...
    public KnightsTour(int rowStart, int colStart){
    ...
    public boolean runTour(){
        ...
        while(fullTour && moveCount < 64){
            fullTour = makeMove();
            System.out.println("Number of the move made: " + (moveCount -1));   // the reason why -1 is here is so that we don't count placing the knight on the spot as a move
            printBoard();
            System.out.println();   //prints an empty line to be able to see moves normally when printed
    ...
    public boolean validMove(int row, int col){
        ...
        boolean bottomCheck = row < board.length;
        boolean leftCheck= col >= 0;
        return topCheck && bottomCheck && leftCheck && col < board[row].length;
        */
    }
